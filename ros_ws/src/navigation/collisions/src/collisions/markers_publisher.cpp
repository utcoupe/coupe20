...
geometry_msgs::Quaternion eulerToQuaternion(const Position &pos) noexcept;

MarkersPublisher::MarkersPublisher(ros::NodeHandle &nodeHandle) {
    m_markersPublisher = nodeHandle.advertise<visualization_msgs::Marker>(MARKERS_TOPIC, QUEUE_SIZE);
}
...
bool MarkersPublisher::m_isConnected() const {
    return m_markersPublisher.getNumSubscribers() > 0;
}
...
void MarkersPublisher::m_publishMarker(
    ...
    m_markersPublisher.publish(marker);
}

void MarkersPublisher::m_addSegmentInfoToMarker(const CollisionsShapes::AbstractShape &shape,
                                                visualization_msgs::Marker &marker) const {
    const auto &segment = dynamic_cast<const CollisionsShapes::Segment &>(shape);

    marker.type = marker.CUBE;
    marker.scale.x = segment.getLength();
    marker.scale.y = 0.02;
}

void MarkersPublisher::m_addRectangleInfoToMarker(const CollisionsShapes::AbstractShape &shape,
                                                  visualization_msgs::Marker &marker) const {
    const auto &rectangle = dynamic_cast<const CollisionsShapes::Rectangle &>(shape);
...
void MarkersPublisher::m_addCircleInfoToMarker(const CollisionsShapes::AbstractShape &shape,
                                               visualization_msgs::Marker &marker) const {
    const auto &circle = dynamic_cast<const CollisionsShapes::Circle &>(shape);

    marker.type = marker.CYLINDER;
    marker.scale.x = marker.scale.y = circle.getRadius() * 2.0;
}
...
geometry_msgs::Quaternion eulerToQuaternion(const Position &pos) noexcept {
    const double cr = std::cos(pos.getX() * 0.5);
    const double sr = std::sin(pos.getX() * 0.5);
    const double cp = std::cos(pos.getY() * 0.5);
    const double sp = std::sin(pos.getY() * 0.5);
    const double cy = std::cos(pos.getAngle() * 0.5);
    const double sy = std::sin(pos.getAngle() * 0.5);
