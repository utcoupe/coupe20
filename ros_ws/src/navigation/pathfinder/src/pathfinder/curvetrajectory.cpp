...
std::vector<Point> CurveTrajectory::compute(double tStep)
{
    // From:
    ...
    if (_points.size() >= 4)
    {
        ...
        for (int i = 0; i < curveNb; i++)
        {
            std::vector<Point> bezierCurve = bezierCurves[i].compute(tStep);
...
void CurveTrajectory::setPoints(std::vector<Point> points)
{
    _points = points;
}


std::vector<Point> CurveTrajectory::getPathPoints()
{
    return _pathPoints;
}
