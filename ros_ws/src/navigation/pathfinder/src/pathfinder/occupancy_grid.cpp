...
pathfinder::OccupancyGrid::OccupancyGrid(const PosConvertor &convertor, size_t nrow, size_t ncol) : _convertor(convertor), _grid(nrow, vector<bool>(ncol, false))
{
}
...
void pathfinder::OccupancyGrid::setOccupancyFromMap(
    const vector<static_map::MapObject> &objects,
    bool clearBefore,
    double safetyMargin,
    const TagsList &ignoredTags)
{
...
bool pathfinder::OccupancyGrid::objectIgnored(const TagsList &objectTags, const TagsSet &tagsToIgnore)
{
