...
CollisionsSubscriptions::RobotPtr CollisionsSubscriptions::createRobot(ros::NodeHandle &nodeHandle) {
    std::lock_guard<std::mutex> lock(m_mutexRobot);
    double width, height;
    auto success = false;
    try {
        ros::ServiceClient srvGetContext = nodeHandle.serviceClient<static_map::MapGetContext>(MAP_GET_CONTEXT_SERVER);
        ...
        if (!srvGetContext.call(srv)) {
            ...
            switch (shape.shape_type) {
                ...
                default:
                    ROS_FATAL_ONCE(
                            "PathfinderROSInterface::_updateMarginFromStaticMap(): Unknown shape."
...
Position CollisionsSubscriptions::m_updateRobotPos() {
    ...
    try {
        ...
        ROS_ERROR_ONCE(
                "An unknown exception happened when trying to update robot's position from tf."
